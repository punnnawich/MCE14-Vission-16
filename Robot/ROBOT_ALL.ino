...
float profile_vel(float t) {
  if (t <= tacce)
    ...
    return prof_vmax - prof_a * (t - tacce);
  else
    return 0.0f;
}
...
bool comms_receive() {
  ...
  // Handle CAMERA_READY (extra == 2) before sequence check to allow camera
  // restarts
  if (pkt.extra == 2) {
    ...
    Serial.println("[RX] CAMERA_READY");
    return true;
  }
...
void enter_wait_camera() {
  ...
  t_wait_camera = millis();
  state = S_WAIT_CAMERA;
  setLED(true, false, true);
  Serial.println("[SM] -> S_WAIT_CAMERA");
}
...
void loop() {
  ...
  if (cnt % 10 == 0) {
    cnt = 0;
    Serial.printf("%.3f,%.3f,%.3f,%.2f,%.3f,%.2f\n", actual_pos1, actual_pos2,
                  actual_pos3, yaw * (180.0f / PI), theta_corr,
                  targetYaw * (180.0f / PI));
  }
  ...
  // ═════════════════════════════
  // S_WAIT_CAMERA
  // รอรับ CAMERA_READY (extra == 2) จากกล้องก่อนเข้า CHECK_HOME
  // LED: เหลืองและแดงกะพริบพร้อมกัน
  // ═════════════════════════════
  case S_WAIT_CAMERA: {
    updateBlinkLED(LED_YELLOW, 500);
    digitalWrite(LED_RED, ledBlinkOn ? HIGH : LOW);

    // Send "WAITING_FOR_CAMERA" every 2.0 seconds
    static uint32_t last_wait_ping = 0;
    if (millis() - last_wait_ping >= 2000) {
      last_wait_ping = millis();
      if (!has_learned_pc_ip)
        comms_send_broadcast("WAITING_FOR_CAMERA");
      else
        comms_send("WAITING_FOR_CAMERA");
    }

    // Timeout: if camera never responds within 120s, proceed anyway
    if (millis() - t_wait_camera >= 120000) {
      Serial.println("[SM] S_WAIT_CAMERA: timeout — proceeding without CAMERA_READY");
      digitalWrite(LED_RED, LOW);
      digitalWrite(LED_YELLOW, LOW);
      enter_check_home();
      break;
  ...
  case S_CHECK_HOME:
    ...
    if (xSemaphoreTake(xMutex, 0) == pdTRUE) {
      if (new_robot_pos) {
        ...
        digitalWrite(LED_RED, LOW);
        float err_cm = sqrtf(rx * rx + ry * ry) * 100.0f;
